...
## 5.8.0 (2026-09-04)
7 skill updates synced from internal source (changes since 2026-08-28), sanitized. +219 / −3 lines. 51 skills total.

### Quality & Verification
- `/review-impl` — **new Phase 1b: probe the worktree when the implementer was an agent.** Reading a diff tells you what changed, not whether it is right, and this phase is what turns a review that only reads into one that executes. Four beats: **fix the base first** — read the agent's actual base commit from its own log rather than from your idea of the integration branch's tip, because a worktree branched from an older commit makes every intervening commit read as the agent's own work; write probes in a scratch dir and **not** in the worktree, so the tree stays exactly as it will merge, each probe answering one behavioral question; **mutate what the agent's tests claim to pin** — copy the module aside, break the specific behavior a test names, and confirm the test goes red, because a test that stays green under mutation is not coverage; and run build/test/lint yourself, since a result the agent *reported* is not one you observed
- `/pre-merge-review` — a **Fix Application** phase, so the deliverable is a merge-ready branch rather than a findings list: verify each finding before fixing it, triage into apply / flag / outward-facing, fix on a branch cut from the reviewed commit with a regression test that fails against the unfixed code, then re-run the full pre-flight and report what was applied versus held. Plus two security patterns — a guard and the surface that resolves it drifting onto different cost bases, and an auth fence proven on one branch but never on the default or fallback path — and a note that a secret scanner needs base64url and UUID coverage alongside false-positive tuning

### Development Pipeline
- `/throwaway-script` — new **"grounding a claim"** section: the rule that a repro built from inputs you invented can only confirm your hypothesis, never falsify it, needs a mechanism, and this is it. One short script per claim, written to *disprove* that claim, run against the real source. Two surfaces — a standalone script for live external data, and a disposable probe test for the behavior of a project module, which needs the project's own resolution to import it. **Name the script after the claim it attacks, not the subject**: the name is the record of what you tried to break. Probe tests are disposable and must be deleted, prefixed so the set is batch-removable, because leaving one behind ships an unreviewed test. And gate the *document* on the scripts — commit a plan or decision doc only once every claim has had a script aimed at it and survived; a claim no script attacked is a hypothesis, so write it as one
- `/throwaway-script` — fixed a self-contradiction the skill had been carrying: the location table said to prefer the session scratchpad while the anti-patterns section twice prescribed a legacy temp directory instead. The table wins. The anti-pattern now names the real trap rather than prescribing it — on Windows the shell's `/tmp` and Node's `C:\tmp` are **different directories**, so a file written through one and read through the other fails with ENOENT
- `/write-plan` — a gate's `CHECK:` must name a command that **exists when the gate is claimed**. A lint that validates the oracle's *shape* cannot tell whether the binary or path resolves, so a phantom check passes lint and then fails, or is never run at all, at claim time. If the plan's own work creates the test, say so in the gate body and leave the evidence pending rather than inventing a script name and hoping. Also cross-references the intent handback below
- `/research-gate` — Phase 0 now also checks for a decided Intent Summary handed back from a prior interrogation session before dispatching any research agent. A remote or chat-surface session cannot continue the gate itself, so it leaves a durable record; not looking for it means re-deriving decisions someone already made

### Session Management
- `/grill-me` — a **chat-surface mode** (pace to one or two questions per turn when the surface is a phone or chat client rather than a terminal) and a **handback** section: paste the Intent Summary back in the same session, or, when the interrogation happened somewhere that cannot continue the work, write it to a durable file on a branch with a `proposed` / `decided` / `consumed` lifecycle so the next session can pick it up
- `/start-day` — two principles worth applying to any morning check. **A verifier's exit code can encode more than pass/fail**: when one code has several sources, read the printed counts and labels rather than treating non-zero as failure. And **silence-by-design needs its own staleness escalation** — a step that deliberately stays quiet on the common case needs a separate threshold, or a dead upstream source reads exactly like a quiet healthy one

## 5.7.0 (2026-08-28)
5 skill updates + 1 new skill synced from internal source (changes since 2026-08-21), sanitized. +210 / −12 lines. 51 skills total.

### Quality & Verification
- **NEW `/gh-repo-recon`** — read a GitHub repo you haven't cloned, through the API rather than a fetch. Six layers, outside-in: shape and file tree, ranged source reads, cross-repo code search, issues/PRs, **governance and security posture** (rulesets, branch protection, deploy keys, Actions permissions, merge attribution, Dependabot alerts — the layer with no local equivalent), and a blobless clone as the last resort for ancestry questions the API cannot express. Carries five traps worth the skill on their own: `mergeable` is UNKNOWN until the host computes it in the background, so **CI-green is not merge-ready**; a review verdict can be **stale-by-head and reads identically to an uncleared one** unless you compare its recorded head to the PR's current head; deleting a branch on merge can **silently close a PR stacked on it**; a code-search zero-result is weak evidence of absence (the index lags pushes); and a 200 response can be an auth login page rather than a success

### Development Pipeline
- `/research-gate` — **new Phase 0, before any research agent is dispatched: check whether someone is already building this.** An open PR on the topic is the cheapest constraint available and the only one that can make the whole gate unnecessary. The trap this closes: a file-level check on the target branch does NOT answer the question — an unlanded PR has none of its files on the branch it is headed for, so the file check reports "not built" for work that is nearly done and in review. Run both halves, and check every repo the feature could land in
- `/write-plan` — the same Step 0 overlap check before writing a single task, plus a new optional **Gate Ledger** pattern for plans where quiet incompleteness is costly. A `Verify:` line inside a task is prose nothing re-checks; it catches nothing when work is built four-fifths of the way and the last piece is simply never mentioned, because no completion claim was ever made to contradict. The ledger makes each independently-omittable outcome a gate with its own proving command, where a ticked box whose evidence still reads `pending` counts as unmet — so the ledger cannot be closed by editing a character. Four authoring rules included, the load-bearing two being **the check must be able to fail** (test it against a deliberately broken version first) and **never let a check accept a hand-supplied number as its own proof** — a check seeded with invented inputs can only confirm the hypothesis that produced it. Abandoned gates get marked with a reason rather than deleted, so the gap stays visible

### Reference & Conventions
- `/throwaway-script` — new **patch-script** pattern for mechanical source rewrites (a throwaway script that transforms a file, when the transformation itself is the unit of work rather than a dozen single-point edits), and the reason it needs a manual lint step: **a post-edit lint hook keyed to editor-tool calls cannot fire for a file written through `fs` from a shell-invoked process.** The edit lands with zero lint coverage, and a build does not close the gap — it catches syntax and types, never a lint-only rule. Same shape as any tool-matched hook a shell-level write slips past: the hook trusts the tool name, not the file's actual change. Also: gitignore the scratch dir you use, since a repo-local one is exposed to `git add -A` sweeping a leftover script into an unrelated commit

### Session Management
- `/session-notes` — **if you script any part of the close-out, have the script write the "this day is closed" marker itself.** A hand-written final step can be skipped with no trace: every artifact it was meant to certify can exist while the marker never gets written, so the next morning reports a close-out as still owed when it already happened. A script-written marker can refuse itself and name which earlier step failed, which is the only way "the checklist ran" becomes a claim you can trust
- `/start-day` — two additions. Sweep dead session worktrees *before* reading the overlap list, or the graveyard drowns the signal; a worktree survives cleanup for one of three reasons (the session was killed before its cleanup hook fired, an interrupted `add` left a locked tree that removal tooling refuses to touch, or the branch outlived the worktree and was rebuilt on resume), and a sweep is only safe unattended when it requires a clean tree and patch-level landing, not mere ancestry. Second: a PR whose checks sit at `action_required` belongs in the **blocked-on-you** bucket, not the agent-ready queue — some CI platforms hold runs for human approval whenever the PR came from a bot account, so the checks read pending or red while nothing is actually failing and no amount of agent effort clears it

## 5.6.0 (2026-08-21)
8 skill updates + 1 new skill synced from internal source (changes since 2026-08-11), sanitized. +755 / −186 lines. 50 skills total.

### Development Pipeline
- **NEW `/consult`** — get an independent second opinion on a **decision** (not a diff) from a second model or agent with its own repo access. The one rule: **transfer the question, not your context** — handing the consultant your reasoning chain anchors it on your conclusion, so the brief carries the question, constraints, options and evidence (labelled *ran it* / *inferred* / *recalled*), and your own recommendation goes last under a `SEALED` marker to be read only after the consultant has formed its view. A vs-neighbours table routes code diffs to review tooling and continuation of your own work to a handoff. Reading the verdict back: report it as theirs, treat disagreement as the finding, never average the two views
- `/cascade-orchestration` — new anti-pattern: **fan-out disguised as sequential dependency** — parallelizing stages that read each other's output re-feeds redundant context for no real concurrency; the session-notes row now reflects its sequential single-agent rewrite

### Quality & Verification
- `/pre-merge-review` — **audit the deploy targets, not just git.** A credential can hide from every repo-side scanner by never entering the repo: a file never `git add`ed is invisible to the secret-scan lint, the pre-push hook and the host's own scanning at once. Scan deploy targets directly for credential literals and list target-only files (source present on the target with no counterpart in git) — that is the blind spot the literals accumulate in. Never write a credential literal into a maintenance script, even temporarily; resolve through a helper that fails closed, and prefer no credential at all when a read-only path already answers the need. Rotation has a coupled blast radius when scratch copies share the live service credential — check what else shares it first. (Found live: five untracked throwaway scripts on a production share carrying the running service's database password since at least five months earlier)
- `/audit-components` — Props Consistency now **derives the render-site set fresh** instead of consulting a static table: a static list rots silently and a coverage gap reads as complete. Every capitalized JSX tag is a render site (not just a suffix pattern), derive against the remote branch rather than the local checkout, and report the derived count so partial coverage is visible

### Build Orchestration
- `/worktree-guard` — new **Bulk Cleanup** section: locked worktrees block both `remove` and `prune`; `gc.worktreePruneExpire` defaults to 3 months so a stale registration needs `--expire=now`; bulk-delete then a single prune beats one-at-a-time removal; a three-check "dirty ≠ real work" heuristic (`git cherry`, untracked-vs-`ls-tree` novelty, then history); `git worktree remove` never deletes the branch; `git fsck --connectivity-only` for corrupt registrations

### Session Management
- `/session-notes` — restructured around **two modes, and picking the right one is the whole cost story.** Checkpoint (append one block, one tool call) is the default and the only mode the assistant may choose on its own; full close-out runs only on explicit request — a repeated ask, the hour of day and a shipped feature are NOT escalation signals. A checkpoint does not discharge the feature-ship checklist; surface the debt in one line instead of pre-emptively spending the checklist. The seven full-close-out artifact steps moved to `references/close-out-artifacts.md` so a checkpoint never pays for them. New **conflict-marker precheck** before any shared-doc edit: `grep -c '<<<<<<<' "$FILE"`, resolve via `git diff :2:FILE :3:FILE`, abort-and-reset-from-remote as the fast exit — a pull-rebase helper is prevention, not detection, so run the grep even when using one. The old End-of-Session Review is now Mode B Step 0 (final state first)
- `/start-day` — Step 3 verifies yesterday's note was a full close-out and not just a checkpoint; Step 4 adds a memory-index size check (archive, never lossy-condense); Step 8.5 adds "retire what you ticked, not just check it" for derived pipeline views

### Skill Maintenance & Learning
- `/skill-audit` — new **Preflight: which tree are you auditing** — audit an exact tree, not a possibly-stale, dirty or ahead checkout; behind-count is not the gate; fall back to a detached worktree or a ref-grep, and abort rather than report from a stale tree. New **Check 7 (Externally-Synced Skills)** for skills consumed by an external system (attachment counts, export-description key, drift) and **Check 8 (Unaudited Extension Surfaces)** for the global skill/command tree that no repo lint sees (name collisions, hardcoded user paths, the Windows/Git Bash `$HOME` gotcha, clone drift). Artifacts are run-stamped with date+time and refuse to overwrite; removed the phantom `trigger-evals`/`trigger-results` listing no check produces
- `/wisdom` — optional **Step D: a background-agent fleet signal** as a third independent behavioral source (hook-derived instincts and transcript-derived usage reports only ever see the operator's own sessions). Task G matches by **subject, never by string** (key spaces coined independently never match literally), with two numeric guards (no trend from a short window; classify capacity failures out before judging health) and the "not-applied is not proof of absence" gotcha. New **Tier 4: global promotion** — per-item, only when the content names no repo-specific system, expected empty most weeks; a downstream automated consumer does not inherit a global-tree change
